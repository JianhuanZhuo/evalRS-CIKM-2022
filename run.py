...
from evaluation.utils import get_cache_directory
...
if __name__ == '__main__':
    LIMIT = 0
    dataset_cache = os.path.join(get_cache_directory(), 'dataset_cache.pkl')
    if not os.path.exists(dataset_cache):
        print(f"compute cache of dataset to {dataset_cache}")
        dataset = ChallengeDataset(force_download=False)  # note, if YES, the dataset will be donwloaded again
        with open(dataset_cache, 'wb') as fp:
            pickle.dump(dataset, fp)
    else:
        print(f"load cache of dataset from {dataset_cache}")
        with open(dataset_cache, 'rb') as fp:
